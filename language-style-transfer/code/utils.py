...
def get_batch_n(x, x_n, y, word2id, min_len=5):
    pad = word2id['<pad>']
    go = word2id['<go>']
    eos = word2id['<eos>']
    unk = word2id['<unk>']
    ...
    max_len_n = max([len(sent) for sent in x_n])
    max_len_n = max(max_len_n, min_len)
    for i in range(len(x)):  # for each sent in x & x_n
        sent_id = [word2id[w] if w in word2id else unk for w in x[i]]
        l = len(x[i])
        padding = [pad] * (max_len - l)
        go_x.append([go] + sent_id + padding)
        x_eos.append(sent_id + [eos] + padding)
        ...
        sent_id = [word2id[w] if w in word2id else unk for w in x_n[i]]
        l = len(x_n[i])
        padding = [pad] * (max_len_n - l)
        rev_x.append(padding + sent_id[::-1])
...
def get_batches_n(x0, x0_n, x1, x1_n, word2id, batch_size):
    if len(x0) < len(x1):
        x0 = makeup(x0, len(x1))
        x0_n = makeup(x0_n, len(x1))

    if len(x1) < len(x0):
        x1 = makeup(x1, len(x0))
        x1_n = makeup(x1_n, len(x0))

    n = len(x0)

    order0 = range(n)
    z = sorted(zip(order0, x0, x0_n), key=lambda i: len(i[1]))
    order0, x0, x0_n = zip(*z)

    order1 = range(n)
    z = sorted(zip(order1, x1, x1_n), key=lambda i: len(i[1]))
    order1, x1, x1_n = zip(*z)

    batches = []
    s = 0
    while s < n:
        t = min(s + batch_size, n)
        batches.append(get_batch_n(x0[s:t] + x1[s:t], x0_n[s:t] + x1_n[s:t],
                                   [0] * (t - s) + [1] * (t - s), word2id))
        s = t
