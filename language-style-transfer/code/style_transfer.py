...
def transfer(model, decoder, sess, args, vocab, data0, data0_n, data1, data1_n, out_path):
    batches, order0, order1 = get_batches(data0, data0_n, data1, data1_n,
...
if __name__ == '__main__':
    ...
    if args.dev:
        ...
        dev1_n = load_sent(args.dev + '.noised1.1')

    if args.test:
        test0 = load_sent(args.test + '.0')
        test0_n = load_sent(args.test + '.noised1.0')
        test1 = load_sent(args.test + '.1')
        test1_n = load_sent(args.test + '.noised1.1')
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.train:
            batches, _, _ = get_batches_n(train0, train0_n, train1, train1_n, vocab.word2id,
                                          args.batch_size)
            ...
            for epoch in range(1, 1 + args.max_epochs):
                ...
                if args.dev:
                    dev_losses = transfer(model, decoder, sess, args, vocab,
                                          dev0, dev0_n, dev1, dev1_n, args.output + '.epoch%d' % epoch)
        ...
        if args.test:
            test_losses = transfer(model, decoder, sess, args, vocab,
                                   test0, test0_n, test1, test1_n, args.output)
